		...
		if (changedKey != null) {
			...
			if (checkValue(PREF_ANNOTATION_NULL_ANALYSIS, ENABLED)
				...
				if (ask) {
					...
					final PreferenceHighlight highlightNullRef= (PreferenceHighlight) fLabels.get(comboBoxNullRef).getData();
					final Combo comboBoxPotNullRef= getComboBox(PREF_PB_POTENTIAL_NULL_REFERENCE);
					final PreferenceHighlight highlightPotNullRef= (PreferenceHighlight) fLabels.get(comboBoxPotNullRef).getData();
