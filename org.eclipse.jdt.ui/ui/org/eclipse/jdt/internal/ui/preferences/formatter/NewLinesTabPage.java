...
public class NewLinesTabPage extends ModifyDialogTabPage {
	...
	protected void doCreatePreferences(Composite composite, int numColumns) {
		...
		createPref(emptyStatementsGroup, numColumns, "NewLinesTabPage.empty_statement_group.option.remove_unnecessary_semicolon", DefaultCodeFormatterConstants.FORMATTER_REMOVE_UNNECESSARY_SEMICOLON, FALSE_TRUE); //$NON-NLS-1$
		
		final Group userGroup= createGroup(numColumns, composite, FormatterMessages.getString("NewLinesTabPage.existing_breaks_group.title")); //$NON-NLS-1$
		createPref(userGroup, numColumns, "NewLinesTabPage.existing_breaks_group.preserve_existing_line_breaks", DefaultCodeFormatterConstants.FORMATTER_PRESERVE_USER_LINEBREAKS, FALSE_TRUE); //$NON-NLS-1$
	}
