 ...
 */
						...
						if (result.isEmpty()) {
							...
							*/
							if (fStorage instanceof IResource) {
								engine.search(createScope(((IResource)fStorage).getProject()), collector, searchPattern, SubMonitor.convert(monitor, 4));
							}
						} else {
							monitor.worked(1);
						}
	...
	private static TextSearchScope createScope(IResource scope) {
		...
		for (String javaExtension : JavaCore.getJavaLikeExtensions()) {
			fileNamePatternStrings.add("*." + javaExtension); //$NON-NLS-1$
		}
