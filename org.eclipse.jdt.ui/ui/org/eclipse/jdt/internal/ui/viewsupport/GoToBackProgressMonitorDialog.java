...
public class GoToBackProgressMonitorDialog extends IconAndMessageDialog implements IRunnableContext {
	...
	private class MyJob extends Job {
		...
		public MyJob(IRunnableWithProgress operation, Display display) {
			super("Running Operation"); //$NON-NLS-1$
	...
	/**
	 * Internal progress monitor implementation. Wraps the real progress monitor
	 * and intercepts all steps to forwrad them to the progress monitor dialog.
	 */
 ...
 */
public GoToBackProgressMonitorDialog(Shell parent) {
	super(parent);
