...
class HierarchyRunView implements ITestRunView {
	...
	class MenuListener implements IMenuListener {
		...
		public void menuAboutToShow(IMenuManager manager){
			if (fTree.getSelectionCount() > 0) {
				...
				if (testInfo.fStatus == TestRunnerUI.IS_SUITE)
				...
				if (testInfo.fStatus == TestRunnerUI.IS_SUITE)			
					manager.add(new Action("&Goto File"){ 
						public void run(){
							...
							fRunViewContext.goToFile(className, 0);
						} 
					});
				else
					manager.add(new Action("&Goto File"){ 
						public void run(){ 
							fRunViewContext.goToFile(getClassName(), getTestLabel());
						} 
					});	
			}
		}		
	...
	private final Image fOkIcon= TestRunnerUIutils.createImage("icons/ok.gif", getClass());
	...
	private final Image fHierarchyIcon= TestRunnerUIutils.createImage("icons/hierarchy.gif", getClass());
		
	public HierarchyRunView(CTabFolder tabFolder, TestRunnerUI context) {
	...
	private TestInfo getTestInfo() {
		TreeItem[] treeItems;
		if((treeItems= fTree.getSelection()).length == 0) return null;
	...
	private String extractClassName(String testNameString) {
		if (testNameString == null) return null;
		int index= testNameString.indexOf('(');
		if (index < 0) return testNameString;
	...
	private TreeItem findItemByTest(String testName, TreeItem[] treeItem){
		...
		if(treeItem.length == 0) return null;
	...
	private void addListeners() {
		...
		fTree.addMouseMoveListener(new MouseMoveListener() {
			public void mouseMove(MouseEvent e) {
				if (!(e.getSource() instanceof Tree)) return;
