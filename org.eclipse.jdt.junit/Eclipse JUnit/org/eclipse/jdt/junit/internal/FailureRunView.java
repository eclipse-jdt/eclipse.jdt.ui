...
class FailureRunView implements ITestRunView {
	...
	private final Image fErrorIcon= TestRunnerUIutils.createImage("icons/error.gif", getClass());
	private final Image fFailureIcon= TestRunnerUIutils.createImage("icons/failure.gif", getClass());
		...
		menuMgr.addMenuListener(new IMenuListener() {
			...
			public void menuAboutToShow(IMenuManager manager){
				if (fTable.getSelectionCount() > 0) {
					...
					manager.add(new Action("&Goto File"){ public void run(){ fRunViewContext.goToFile(getClassName(), getMethodName());} });
				}
			}		
		});
	...
	protected void addListeners() {
		...
		fTable.addMouseListener(new MouseListener() {
			public void mouseDoubleClick(MouseEvent e){
				fRunViewContext.goToFile(getClassName(), getMethodName());
			}
