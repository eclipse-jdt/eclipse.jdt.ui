...
public class JUnit4TestFinder implements ITestFinder {
	private static class Annotation {
		private static final JUnit4TestFinder.Annotation RUN_WITH = new JUnit4TestFinder.Annotation(
				new String[] { "RunWith", "org.junit.runner.RunWith" });

		private static final JUnit4TestFinder.Annotation TEST = new JUnit4TestFinder.Annotation(
				new String[] { "Test", "org.junit.Test" });
		...
		public boolean foundIn(String source) {
			IScanner scanner = ToolFactory.createScanner(false, true, false,
					false);
				...
				do {
					...
					if (tok == ITerminalSymbols.TokenNameAT) {
						...
						while (tok == ITerminalSymbols.TokenNameIdentifier
							...
							annotationName += String.valueOf(scanner
	...
	public void findTestsInContainer(IJavaElement container, Set result,
			IProgressMonitor pm) {
		try {
	...
	private void findTestsInProject(IJavaProject project, Set result)
	...
	public boolean isTest(IType type) throws JavaModelException {
		return Annotation.RUN_WITH.annotates(type)
				|| Annotation.TEST.annotatesAtLeastOneMethod(type);
	}
}
