...
public class JobManager implements Runnable {
	 ...
	 */
	public static final int FORCE_IMMEDIATE = 1;
	/**
	 * The search operation throws an <code>org.eclipse.core.runtime.OperationCanceledException</code>
	 * if the underlying indexer has not finished indexing the workspace.
	 */
	public static final int CANCEL_IF_NOT_READY = 2;
	/**
	 * The search operation waits for the underlying indexer to finish indexing 
	 * the workspace before starting the search.
	 */
	public static final int WAIT_UNTIL_READY = 3;
	
	/* queue of jobs to execute */
	protected Job[] awaitingJobs = new Job[10];
	 ...
	 */
	public synchronized Job currentJob() {
	...
	public boolean performConcurrentJob(
		...
		if (VERBOSE)
			JobManager.verbose("STARTING  concurrent job - " + concurrentJob); //$NON-NLS-1$
		if (!concurrentJob.isReadyToRun()) {
			if (VERBOSE)
				JobManager.verbose("ABORTED   concurrent job - " + concurrentJob); //$NON-NLS-1$
			return false;
		}
		...
		if (awaitingJobsCount() > 0) {
			switch (waitingPolicy) {
				...
				case FORCE_IMMEDIATE :
					if (VERBOSE)
						JobManager.verbose("-> NOT READY - forcing immediate - " + concurrentJob);//$NON-NLS-1$
					boolean wasEnabled = isEnabled();
					try {
						disable(); // pause indexing
						status = concurrentJob.run(progress == null ? null : new SubProgressMonitor(progress, concurrentJobWork));
					} finally {
						if (wasEnabled)
							enable();
					}
					if (VERBOSE)
						JobManager.verbose("FINISHED  concurrent job - " + concurrentJob); //$NON-NLS-1$
					return status;
					
				case CANCEL_IF_NOT_READY :
					if (VERBOSE)
						JobManager.verbose("-> NOT READY - cancelling - " + concurrentJob); //$NON-NLS-1$
					if (progress != null) progress.setCanceled(true);
					if (VERBOSE)
						JobManager.verbose("CANCELED concurrent job - " + concurrentJob); //$NON-NLS-1$
					...
					try {
						...
						while ((awaitingWork = awaitingJobsCount()) > 0) {
							...
							if (currentJob != null && currentJob != previousJob) {
								if (VERBOSE)
									JobManager.verbose("-> NOT READY - waiting until ready - " + concurrentJob);//$NON-NLS-1$
								if (subProgress != null) {
									subProgress.subTask(
	...
	public synchronized void request(Job job) {
		if (!job.isReadyToRun()) {
			if (VERBOSE)
