...
public final class CoreASTProvider {
	...
	public CompilationUnit getAST(final ITypeRoot input, WAIT_FLAG waitFlag, IProgressMonitor progressMonitor) {
		return getAST(input, waitFlag, progressMonitor, Instant.now().plus(getOverallReconcileTimeoutMilliseconds(), ChronoUnit.MILLIS));
	}
	 ...
	 */
	private CompilationUnit getAST(final ITypeRoot input, WAIT_FLAG waitFlag, IProgressMonitor progressMonitor, Instant reconcilingDeadline) {
		...
		if (isReconciling) {
			if (Instant.now().isAfter(reconcilingDeadline)) {
			...
			try {
				...
				synchronized (fWaitLock) {
					if (isReconciling(input)) {
						...
						long remainingMillis= reconcilingDeadline.toEpochMilli() - System.currentTimeMillis();
				...
				return getAST(input, waitFlag, progressMonitor, reconcilingDeadline);
