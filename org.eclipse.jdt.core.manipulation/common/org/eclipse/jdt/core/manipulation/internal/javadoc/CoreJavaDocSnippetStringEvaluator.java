...
public class CoreJavaDocSnippetStringEvaluator {
	...
	public enum ReplacementStringIntervalStatus {
		...
		ENCOMPASS,
		/**
		 * replacement interval starts before the given ActionElement interval and ends within the
		 * ActionElement interval
		 */
		PREV_OVERLAP,
		/**
		 * replacement interval starts within the given ActionElement interval and ends after the
		 * ActionElement interval
		 */
		POST_OVERLAP,
		/**
	...
	public void AddTagElementString(TagElement snippetTag, StringBuffer buffer) {
		if (snippetTag != null) {
			...
			for (Object fragment : fragments) {
				buffer.append(getOneTagElementString(snippetTag, fragment));
			}
		}
	}
	...
	protected String getModifiedStringForTagElement(TagElement tagElement, List<TagElement> tagElements) {
		return getModifiedString(tagElement, tagElements);
	}
	...
	private void handleSnippetHighlight(String text, TagElement tagElement, List<ActionElement> actionElements) {
		try {
			...
			String startDefHighlight= getStartTag(defHighlight);
			String endDefHighlight= getEndTag(defHighlight);
	...
	private String getLinkRef(ASTNode node) {
		...
		if (refTypeName != null) {
			...
			try {
				...
				String uri= CoreJavaElementLinks.createURI(scheme, fElement, refTypeName, refMemberName, refMethodParamTypes);
				str+= uri;
	...
	protected String getLinkHtmlTag(List<? extends ASTNode> tagProperties) {
		return getLinkHtmlTag(tagProperties, "code"); //$NON-NLS-1$
	}
	...
	private String getHighlightHtmlTag(List<? extends ASTNode> tagProperties) {
		String defaultTag= getDefaultBoldTag();
		if (tagProperties != null) {
			for (ASTNode node : tagProperties) {
				if (node instanceof TagProperty) {
					TagProperty tagProp= (TagProperty) node;
					if ("type".equals(tagProp.getName())) { //$NON-NLS-1$
						...
						switch (tagValue) {
							case "bold": //$NON-NLS-1$
								defaultTag= getDefaultBoldTag();
								break;
							case "italic": //$NON-NLS-1$
								defaultTag= getDefaultItalicTag();
								break;
							case "highlighted": //$NON-NLS-1$
								defaultTag= getDefaultHighlightedTag();
								break;
							default:
								defaultTag= ""; //$NON-NLS-1$
								break;
