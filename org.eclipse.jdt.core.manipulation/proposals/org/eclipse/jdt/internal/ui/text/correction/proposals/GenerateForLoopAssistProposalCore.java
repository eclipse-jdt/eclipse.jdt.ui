 ...
 *     Red Hat Inc. - Body moved from org.eclipse.jdt.internal.ui.text.correction.proposals.GenerateForLoopAssistProposal
 *******************************************************************************/
...
public class GenerateForLoopAssistProposalCore extends LinkedCorrectionProposalCore {
	...
	public GenerateForLoopAssistProposalCore(ICompilationUnit cu, ExpressionStatement currentStatement, int loopTypeToGenerate) {
		super("", cu, null, IProposalRelevance.GENERATE_FOR_LOOP); //$NON-NLS-1$
		...
		switch (loopTypeToGenerate) {
			case GenerateForLoopAssistProposalCore.GENERATE_FOREACH:
				...
				break;
			case GenerateForLoopAssistProposalCore.GENERATE_ITERATOR_FOR:
				setDisplayName(CorrectionMessages.QuickAssistProcessor_generate_iterator_for_loop);
				break;
			case GenerateForLoopAssistProposalCore.GENERATE_ITERATE_ARRAY:
				setDisplayName(CorrectionMessages.QuickAssistProcessor_generate_for_loop);
				break;
			case GenerateForLoopAssistProposalCore.GENERATE_ITERATE_LIST:
	...
	private SimpleName resolveLinkedVariableNameWithProposals(ASTRewrite rewrite, String basename, String excludedName, boolean firstLinkedProposal) {
		...
		for (String nameProposal : nameProposals) {
			addLinkedPositionProposal(forDeclarationName.getIdentifier(), nameProposal);
		}
