	...
	public RefactoringStatus checkFinalConditions(final IProgressMonitor pm) throws CoreException, OperationCanceledException {
		...
		try {
			...
			for (Entry<IJavaProject, ArrayList<IJavaElement>> entry : projectsToElements.entrySet()) {
				...
				IProgressMonitor projectMonitor= SubMonitor.convert(pm, 1);
				...
				for (int i= 0; i < batches; i++) {
					...
					final IProgressMonitor batchMonitor= SubMonitor.convert(projectMonitor, 1);
...
//			});
			...
			InferTypeArgumentsUpdate updates= solver.solveConstraints(SubMonitor.convert(pm, 1));
			solver= null; //free caches

			fChangeManager= new TextChangeManager();
			rewriteDeclarations(updates, SubMonitor.convert(pm, 1));
