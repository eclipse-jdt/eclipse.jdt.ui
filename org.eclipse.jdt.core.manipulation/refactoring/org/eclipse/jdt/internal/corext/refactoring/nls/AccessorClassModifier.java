...
public class AccessorClassModifier {
	...
	private AccessorClassModifier(ICompilationUnit cu) throws CoreException {
		...
		if (fRoot.types().size() > 0) {
			...
			IStatus status= new Status(IStatus.ERROR, JavaManipulation.ID_PLUGIN, IJavaStatusConstants.INTERNAL_ERROR, NLSMessages.AccessorClassModifier_missingType, null);
