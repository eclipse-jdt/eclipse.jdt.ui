...
public abstract class HierarchyProcessor extends SuperTypeRefactoringProcessor {
	...
	protected RefactoringStatus checkConstructorCalls(final IType type, final IProgressMonitor monitor) throws JavaModelException {
		try {
			...
			for (SearchResultGroup group : ConstructorReferenceFinder.getConstructorReferences(type, fOwner, SubMonitor.convert(monitor, 1), result)) {
				ICompilationUnit unit= group.getCompilationUnit();
				if (unit != null) {
					final CompilationUnit cuNode= RefactoringASTParser.parseWithASTProvider(unit, false, SubMonitor.convert(monitor, 1));
