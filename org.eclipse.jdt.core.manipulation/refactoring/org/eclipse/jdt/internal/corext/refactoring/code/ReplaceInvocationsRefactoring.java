...
//			case ASTNode.CONSTRUCTOR_INVOCATION:
	...
	public RefactoringStatus checkFinalConditions(IProgressMonitor pm) throws CoreException {
		...
		ICompilationUnit[] units= fTargetProvider.getAffectedCompilationUnits(searchStatus, binaryRefs, SubMonitor.convert(pm, 1));
		...
		if (result.hasFatalError())
			return result;
		result.merge(ResourceChangeChecker.checkFilesToBeChanged(filesToBeModified, SubMonitor.convert(pm, 1)));
		checkOverridden(result, SubMonitor.convert(pm, 4));
		IProgressMonitor sub= SubMonitor.convert(pm, 15);
		...
		for (ICompilationUnit unit : units) {
			...
			try {
				...
				change.setEdit(root);
				BodyDeclaration[] bodies= fTargetProvider.getAffectedBodyDeclarations(unit, SubMonitor.convert(pm, 1));
				if (bodies.length == 0)
					continue;
				...
				for (BodyDeclaration body : bodies) {
					...
					ASTNode[] invocations= removeNestedCalls(nestedInvocations, unit,
						fTargetProvider.getInvocations(body, SubMonitor.convert(sub, 2)));
	...
	private void checkOverridden(RefactoringStatus status, IProgressMonitor pm) throws JavaModelException {
		...
		ITypeHierarchy hierarchy= type.newTypeHierarchy(SubMonitor.convert(pm, 6));
		checkSubTypes(status, method, hierarchy.getAllSubtypes(type), SubMonitor.convert(pm, 1));
		checkSuperClasses(status, method, hierarchy.getAllSuperclasses(type), SubMonitor.convert(pm, 1));
		checkSuperInterfaces(status, method, hierarchy.getAllSuperInterfaces(type), SubMonitor.convert(pm, 1));
