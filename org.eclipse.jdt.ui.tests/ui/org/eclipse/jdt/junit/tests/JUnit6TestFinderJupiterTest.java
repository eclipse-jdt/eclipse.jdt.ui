...
/**
 * Test if the <code>JUnit5TestFinder</code> can find tests annotated with the API of JUnit6
 * (Jupiter)
 * <p><b>Note:</b> We re-use <code>JUnit5TestFinder</code> for JUnit 6.</p>
 */
@RunWith(Parameterized.class)
public class JUnit6TestFinderJupiterTest {
	...
	public static void beforeClass() throws Exception {
		...
		JavaProjectHelper.addRTJar_17(javaProject, false);
		IClasspathEntry cpe= JavaCore.newContainerEntry(JUnitCore.JUNIT6_CONTAINER_PATH);
		JavaProjectHelper.addToClasspath(javaProject, cpe);
		JavaProjectHelper.set17CompilerOptions(javaProject, false);
	...
	private static ICompilationUnit createCompilationUnit(IPackageFragment packageFragment) throws Exception {
		try (InputStream stream= JUnit6TestFinderJupiterTest.class.getResourceAsStream(TEST_CLASS_FILE);) {
